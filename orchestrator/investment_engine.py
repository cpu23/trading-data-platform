...
STANDARD_METRICS = {
    "revenue", "operating_cash_flow", "capex", "fcf", "fcf_margin", "gross_margin", "gross_profit",
    "net_income", "diluted_eps", "shares_outstanding", "net_debt", "ebitda",
    "inventory", "backlog", "total_debt", "total_assets", "total_liabilities",
    "equity", "current_assets", "current_liabilities",
}

SIGNAL_RULES = OrderedDict({
    "revenue": {"weight": 2, "rule": "change >= 10%: +2; change > 0%: +1; change < -10%: -2; otherwise -1"},
    "capex": {"weight": 2, "rule": "capex growth >= 15%: +2; growth > 0%: +1; decline < -10%: -2; otherwise -1"},
    "fcf": {"weight": 2, "rule": "fcf margin expansion: +2; growth: +1; margin contraction: -2; decline: -1"},
    "gross_margin_delta": {"weight": 1, "rule": "gross margin >= 60% or expansion: +1; contraction: -1"},
    "inventory_vs_revenue": {"weight": 2, "rule": "inventory growth > revenue growth + 10%: -2; in line: +1"},
    "backlog": {"weight": 1, "rule": "backlog growth > 0%: +1; decline: -1"},
    "ai_demand": {"weight": 2, "rule": "qualitative ai demand: strong=+2, moderate=+1"},
    "data_centre_demand": {"weight": 1, "rule": "qualitative datacenter demand: strong/moderate=+1"},
    "supply_constraints": {"weight": 1, "rule": "tight supply / structural shortages: +1"},
    "pricing_power": {"weight": 1, "rule": "pricing power / rising ASPs: +1"},
    "guidance_direction": {"weight": 2, "rule": "raised guidance: +2; in-line: +1; cut: -2; maintained/flat: 0"},
})
...
def _extract_metric(facts: Mapping[str, Any], name: str) -> tuple[float | None, dict[str, Any]]:
...
def _signal(rule_name: str, score: int, observed: Any, prior: Any, evidence: Any, *, basis: str = "deterministic_metric", comparable: bool | None = None) -> dict[str, Any]:
    ...
    if comparable is None:
        comparable = (obs_n is not None and pri_n is not None) if basis == "deterministic_metric" else False
    chg = (obs_n - pri_n) if obs_n is not None and pri_n is not None else None
    chg_pct = _pct_change(obs_n, pri_n)
    ev_list = [evidence] if isinstance(evidence, (str, dict)) else (list(evidence) if isinstance(evidence, (list, tuple)) else [])
    return {
...
def _dcf(fcf: float | None, growth: float | None, dr: float, tg: float) -> dict[str, Any] | None:
    ...
    return {
        ...
        "forecast": forecast,
        "assumptions": {"discount_rate": dr, "terminal_growth": tg, "inferred_growth": growth},
    }


def _dcf_case(fcf: float | None, growth: float, dr: float, tg: float, net_debt: float | None, shares: float | None) -> dict[str, Any] | None:
    ...
    return {"enterprise_value": ev, "equity_value": eq_val, "per_share": ps, "assumptions": d["assumptions"]}


def _valuation_sensitivity(base_fcf: float | None, base_g: float | None, base_dr: float, base_tg: float, net_debt: float | None, shares: float | None) -> dict[str, Any]:
    if base_fcf is None or base_fcf <= 0 or base_g is None or base_dr <= base_tg:
        return {"status": "unavailable", "reason": "positive base cash flow and valid assumptions required", "wacc_terminal_grid": [], "drivers": {}, "range": {"enterprise_value_min": None, "enterprise_value_max": None, "per_share_min": None, "per_share_max": None}, "largest_range_driver": None}
        ...
        for tg in [base_tg - 0.005, base_tg, base_tg + 0.005]:
            if dr > tg:
                ...
                if c:
                    ...
                    grid.append({"discount_rate": dr, "terminal_growth": tg, "enterprise_value": c["enterprise_value"], "per_share": c["per_share"]})
    
    drivers = {}
    spreads = {}
    param_ranges = {
        "starting_fcf": [(base_fcf * 0.9, base_g, base_dr, base_tg), (base_fcf * 1.1, base_g, base_dr, base_tg)],
        "annual_growth": [(base_fcf, base_g * 0.8, base_dr, base_tg), (base_fcf, base_g * 1.2, base_dr, base_tg)],
        "discount_rate": [(base_fcf, base_g, base_dr + 0.01, base_tg), (base_fcf, base_g, max(0.001, base_dr - 0.01), base_tg)],
        "terminal_growth": [(base_fcf, base_g, base_dr, max(0.0, base_tg - 0.005)), (base_fcf, base_g, base_dr, base_tg + 0.005)],
    }
    for pname, (low_p, high_p) in param_ranges.items():
        ...
        if c_low and c_high:
            ...
            drivers[pname] = {
                "enterprise_value_low": min(c_low["enterprise_value"], c_high["enterprise_value"]),
                "enterprise_value_high": max(c_low["enterprise_value"], c_high["enterprise_value"]),
                "per_share_low": min(c_low["per_share"], c_high["per_share"]) if c_low["per_share"] and c_high["per_share"] else None,
                "per_share_high": max(c_low["per_share"], c_high["per_share"]) if c_low["per_share"] and c_high["per_share"] else None,
            }
    ...
    return {
        ...
        "reason": None if has_ps else "net debt and positive shares are required for per-share sensitivity",
...
class MaterialRelationship:
    ...
    def to_payload(self) -> dict[str, Any]:
        return {
            ...
            "material_relationships": [r.to_payload() for r in self.material_relationships],
        }
...
class MaterialRelationshipContract:
    ...
    def to_payload(self) -> dict[str, Any]:
        return {
            "relationship_facts": {f.fact_id: f.to_payload() for f in self.relationship_facts},
            "material_relationships": [r.to_payload() for r in self.material_relationships],
        }


def build_material_relationship_contract(current_facts: Mapping[str, Any], prior_facts: Mapping[str, Any] | None = None) -> MaterialRelationshipContract:
    raw_curr = _mapping(current_facts)
    curr_m = raw_curr.get("metrics") if isinstance(raw_curr.get("metrics"), Mapping) else raw_curr
    facts_dict: dict[str, NormalizedRelationshipFact] = {}
    relationships: list[MaterialRelationship] = []

    def get_or_add_fact(key: str, rec: Mapping[str, Any], default_period: str = "FY2025") -> NormalizedRelationshipFact | None:
    ...
    if isinstance(rev_g, Mapping) and isinstance(ni_g, Mapping):
        ...
        if f_rev and f_ni:
            relationships.append(MaterialRelationship(
        ...
        if f_ocf and f_cpx:
            relationships.append(MaterialRelationship(
                ...
                required_fact_refs=(
                    ...
                    RelationshipFactRef(f_cpx.fact_id, "capital_expenditure", "flow"),
                ),
            ))
            ...
            if tags.get("role") == "external_effect":
                ...
                if f_ext:
                    relationships.append(MaterialRelationship(
                        ...
                        priority=3,
                        required_fact_refs=(RelationshipFactRef(f_ext.fact_id, "external_effect", tags.get("leaf", "effect")),),
                    ))

    return MaterialRelationshipContract(
        relationship_facts=tuple(facts_dict.values())[:MAX_NORMALIZED_RELATIONSHIP_FACTS],
...
def build_deterministic_analysis(
    ...
    *,
    ...
    pri_calc_fcf = (pri_ocf - pri_cpx) if (pri_ocf is not None and pri_cpx is not None) else None
    ...
    for k in all_keys:
        if k == "fcf":
            c_val = fcf
            p_val = pri_fcf
            src = fcf_rec.get("source") or ("reported" if explicit_fcf is not None else "derived")
            rec = fcf_rec
        elif k == "fcf_margin":
            c_val = ((fcf / rev) * 100.0) if (fcf is not None and rev and rev > 0) else None
            p_val = ((pri_fcf / pri_rev) * 100.0) if (pri_fcf is not None and pri_rev and pri_rev > 0) else None
            src = "derived"
            rec = {}
        elif k == "gross_margin":
        ...
        if c_val is not None or p_val is not None or k in STANDARD_METRICS:
            ...
            entry: dict[str, Any] = {
                ...
                "change": chg,
                "pct_change": chg_pct,
                "unit": rec.get("unit", "percent" if "margin" in k or "growth" in k else "currency"),
    ...
    # 2. Valuation
    price = _finite(mkt.get("market_price")) or _finite(curr_m.get("market_price", {}).get("value"))
    shares = _finite(mkt.get("shares_outstanding")) or _finite(curr_m.get("shares_outstanding", {}).get("value"))
    net_debt = _finite(mkt.get("net_debt")) or _finite(curr_m.get("net_debt", {}).get("value"))
    mcap_override = _finite(mkt.get("market_cap"))
    mcap = mcap_override if mcap_override is not None else ((price * shares) if (price and shares) else None)
    ...
    pe = (price / eps) if (price and eps and eps > 0) else ((mcap / ni) if (mcap and ni and ni > 0) else None)
    ...
    inferred_g = min(0.20, max(0.0, ((fcf - pri_fcf) / abs(pri_fcf)))) if (fcf and pri_fcf and pri_fcf > 0) else 0.05
    dcf_base = _dcf(fcf, inferred_g, dr, tg)
    dcf_ev = dcf_base["enterprise_value"] if dcf_base else None
    dcf_eq = (dcf_ev - net_debt) if (dcf_ev is not None and net_debt is not None) else None
    dcf_ps = (dcf_eq / shares) if (dcf_eq is not None and shares and shares > 0) else None

    bull = _dcf_case(fcf, min(0.25, inferred_g * 1.2), max(0.06, dr - 0.01), tg + 0.005, net_debt, shares)
    bear = _dcf_case(fcf, max(0.0, inferred_g * 0.8), dr + 0.01, max(0.01, tg - 0.005), net_debt, shares)
    ...
    dcf_status = "calculated" if dcf_ps is not None else ("enterprise_value_only" if dcf_ev is not None else "unavailable")
    ...
    valuation = {
        ...
        "base": {"enterprise_value": dcf_ev, "equity_value": dcf_eq, "per_share": dcf_ps, "assumptions": dcf_assumptions},
        "bull": bull or {"enterprise_value": None, "equity_value": None, "per_share": None, "assumptions": {}},
        "bear": bear or {"enterprise_value": None, "equity_value": None, "per_share": None, "assumptions": {}},
        "assumptions": dcf_assumptions,
    }
    ...
    rev_score = 2 if (rev_pct is not None and rev_pct >= 10) else (1 if (rev_pct is not None and rev_pct > 0) else (-2 if (rev_pct is not None and rev_pct < -10) else (0 if rev_pct is None else -1)))
    signals["revenue"] = _signal("revenue", rev_score, rev, pri_rev, rev_rec.get("evidence", []))

    # Capex signal
    cpx_pct = _pct_change(cpx, pri_cpx)
    cpx_score = 2 if (cpx_pct is not None and cpx_pct >= 15) else (1 if (cpx_pct is not None and cpx_pct > 0) else (-2 if (cpx_pct is not None and cpx_pct < -10) else (0 if cpx_pct is None else -1)))
    signals["capex"] = _signal("capex", cpx_score, cpx, pri_cpx, cpx_rec.get("evidence", []))

    # FCF signal
    fcf_m_curr = metrics["fcf_margin"]["value"]
    fcf_m_pri = metrics["fcf_margin"]["prior_value"]
    fcf_score = 2 if (fcf_m_curr is not None and fcf_m_pri is not None and fcf_m_curr > fcf_m_pri) else (1 if (fcf and pri_fcf and fcf > pri_fcf) else (-2 if (fcf_m_curr is not None and fcf_m_pri is not None and fcf_m_curr < fcf_m_pri) else 0))
    signals["fcf"] = _signal("fcf", fcf_score, fcf, pri_fcf, fcf_rec.get("evidence", []))

    # Gross margin delta
    gm_curr = metrics["gross_margin"]["value"]
    gm_pri = metrics["gross_margin"]["prior_value"]
    gm_score = 1 if (gm_curr is not None and (gm_curr >= 60 or (gm_pri is not None and gm_curr > gm_pri))) else (-1 if (gm_curr is not None and gm_pri is not None and gm_curr < gm_pri) else 0)
    signals["gross_margin_delta"] = _signal("gross_margin_delta", gm_score, gm_curr, gm_pri, metrics["gross_margin"]["evidence"])
    ...
    inv_score = -2 if (inv_pct is not None and rev_pct is not None and inv_pct > rev_pct + 10) else (1 if (inv_pct is not None and rev_pct is not None and inv_pct <= rev_pct) else 0)
    signals["inventory_vs_revenue"] = _signal("inventory_vs_revenue", inv_score, inv, pri_inv, inv_rec.get("evidence", []))

    # Backlog
    bl, bl_rec = _extract_metric(curr, "backlog")
    pri_bl, _ = _extract_metric(prior, "backlog")
    bl_pct = _pct_change(bl, pri_bl)
    bl_score = 1 if (bl_pct is not None and bl_pct > 0) else (-1 if (bl_pct is not None and bl_pct < 0) else 0)
    signals["backlog"] = _signal("backlog", bl_score, bl, pri_bl, bl_rec.get("evidence", []))
    ...
    signals["ai_demand"] = _signal("ai_demand", ai_sc, "present" if ai_sc else None, None, ai_ev, basis="report_qualitative")

    dc_sc, dc_ev = eval_qual("data_centre_demand", 1)
    if not dc_sc:
        dc_sc, dc_ev = eval_qual("datacenter_demand", 1)
    signals["data_centre_demand"] = _signal("data_centre_demand", dc_sc, "present" if dc_sc else None, None, dc_ev, basis="report_qualitative")

    sc_sc, sc_ev = eval_qual("supply_constraints", 1)
    signals["supply_constraints"] = _signal("supply_constraints", sc_sc, "present" if sc_sc else None, None, sc_ev, basis="report_qualitative")

    pp_sc, pp_ev = eval_qual("pricing_power", 1)
    signals["pricing_power"] = _signal("pricing_power", pp_sc, "present" if pp_sc else None, None, pp_ev, basis="report_qualitative")
    ...
    if g_up.get("present") or str(g_up.get("strength")).lower() == "raised":
        ...
        g_sc = 0
        g_ev = []
    signals["guidance_direction"] = _signal("guidance_direction", g_sc, "raised" if g_sc > 0 else ("cut" if g_sc < 0 else None), None, g_ev, basis="report_qualitative")
    ...
    mat_covered = {
        ...
        "gross_margin_delta": bool(signals["gross_margin_delta"]["comparable"] or gm_curr is not None),
    }
    ...
    if stage in {"confirmed", "accelerating"} and p_count >= 2 and n_count >= 3 and (pe and pe >= 25):
        stage = "mature"

    prev_st = _mapping(previous_state).get("stage") or str(previous_state or "unassigned")
