    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if isinstance(category_or_problems, str):
            ...
            self.problems = list(problems) if problems is not None else [category_or_problems]
        else:
            self.category = category
            self.problems = list(category_or_problems)
        self.categories = [self.category]
        self.problems_by_category = problems_by_category or {self.category: self.problems}
        super().__init__(f"Investment response validation failed: {'; '.join(self.problems[:3])}")
...
def _canonical_claim_number(value: object) -> Decimal | None:
    ...
    match = re.fullmatch(r"([-+]?\d+(?:\.\d+)?)\s*(trillion|billion|bns|bn|million|mns|mn|thousand|b|m|k|x)?", text)
    if not match:
        return None
...
def material_numeric_tokens(text: str) -> list[tuple[int, int, str]]:
    ...
    for match in _MATERIAL_CALENDAR_DATE_RE.finditer(text):
        try:
            if match.group("iso_year"):
                date(int(match.group("iso_year")), int(match.group("iso_month")), int(match.group("iso_day")))
            elif match.group("named_month"):
                date(int(match.group("named_year")), _ENGLISH_MONTH_NUMBERS[match.group("named_month").casefold()], int(match.group("named_day")))
            else:
                date(int(match.group("leading_year")), _ENGLISH_MONTH_NUMBERS[match.group("leading_month").casefold()], int(match.group("leading_day")))
    ...
    for match in _MATERIAL_NUMERIC_TOKEN_RE.finditer(text):
        ...
        if key is None or (_YEAR_LIKE_TOKEN_RE.fullmatch(raw.removesuffix(",")) and not any(c in raw for c in "$€£¥%")):
            continue
...
def validate_numeric_claim_rows(claims: list[dict] | None) -> list[str]:
    ...
    for i, row in enumerate(claims):
        ...
        if not isinstance(cid, str) or not cid.strip():
            problems.append(f"$.numeric_claims[{i}].claim_id: must be a nonblank string")
        elif cid in seen_ids:
            problems.append(f"$.numeric_claims[{i}].claim_id: duplicate claim_id '{cid}'")
        else:
            seen_ids.add(cid)
        val = row.get("value")
        if val is None or _canonical_claim_number(val) is None:
            problems.append(f"$.numeric_claims[{i}].value: must be a finite number or valid numeric string")
        if not isinstance(row.get("metric"), str) or not str(row.get("metric", "")).strip():
            problems.append(f"$.numeric_claims[{i}].metric: must be a nonblank string")
        if not isinstance(row.get("period"), str) or not str(row.get("period", "")).strip():
            problems.append(f"$.numeric_claims[{i}].period: must be a nonblank string")
        unit, curr = row.get("unit"), row.get("currency")
        if not (unit in NUMERIC_CLAIM_UNITS or (isinstance(curr, str) and curr.strip())):
            problems.append(f"$.numeric_claims[{i}]: must have a valid unit or currency")
    return problems


def investment_evidence_violations(parsed: dict, *, excerpt: str, news_items: object = None) -> list[str]:
    violations: list[str] = []
    ...
    def check(evidence: object, label: str, *, required: bool) -> None:
        ...
        if not any(norm in span for span in spans):
            violations.append(f"{label}: evidence is not grounded in the filing excerpt")

    qualitative = parsed.get("qualitative") if isinstance(parsed.get("qualitative"), dict) else {}
    for name in QUALITATIVE_NAMES:
        item = qualitative.get(name)
        if isinstance(item, dict):
            check(item.get("evidence"), f"qualitative.{name}", required=bool(item.get("present")))
    assessment = parsed.get("materiality_assessment") if isinstance(parsed.get("materiality_assessment"), dict) else {}
    for topic in MATERIALITY_ASSESSMENT_TOPICS:
        item = assessment.get(topic)
        if isinstance(item, dict):
            check(item.get("evidence"), f"materiality_assessment.{topic}", required=item.get("status") == "addressed")
...
def validate_investment_report_payload(facts: dict, *, excerpt: str | None = None) -> list[str]:
...
class InvestmentReport(BaseModel):
    ...
    relationship_reconciliations: list[RelationshipReconciliationItem] = Field(default_factory=list, max_length=40)
        ...
        for i, r in enumerate(risks):
            if isinstance(r, dict):
                ...
                if obs and inf and obs == inf:
                    problems.append(f"$.risks[{i}]: sourced_observation and inference must differ")
        ...
        for i, c in enumerate(catalysts):
            if isinstance(c, dict):
                ...
                if trig and exp and trig == exp:
                    problems.append(f"$.catalysts[{i}]: trigger and expected_outcome must differ")
    return problems
...
def validate_relationship_reconciliations(
    ...
    if not isinstance(reconciliations, list) or len(reconciliations) != len(material_relationships):
        return [f"relationship_reconciliations: expected exactly {len(material_relationships)} ordered rows from the request contract"]
    problems: list[str] = []
    summary_text = _normalize_grounding_text(payload.get("summary"))
    thesis_text = _normalize_grounding_text(payload.get("thesis"))
    for index, (expected_rel, row) in enumerate(zip(material_relationships, reconciliations, strict=False)):
        if not isinstance(row, dict):
            problems.append(f"relationship_reconciliations[{index}]: must be an object")
            continue
        rel_id = expected_rel.get("relationship_id") if isinstance(expected_rel, Mapping) else getattr(expected_rel, "relationship_id", None)
        expected_compat = expected_rel.get("compatibility") if isinstance(expected_rel, Mapping) else getattr(expected_rel, "compatibility", None)
        expected_paths = expected_rel.get("fact_paths") if isinstance(expected_rel, Mapping) else getattr(expected_rel, "fact_paths", None)
        if expected_paths is None and isinstance(expected_rel, Mapping) and "required_facts" in expected_rel:
            expected_paths = tuple(
                rf.get("fact_path") if isinstance(rf, Mapping) else getattr(rf, "fact_path", str(rf))
        ...
        if row.get("relationship_id") != rel_id:
            problems.append(f"relationship_reconciliations[{index}].relationship_id: must equal request relationship {rel_id!r} at this position")
        if tuple(row.get("fact_paths") or ()) != tuple(expected_paths):
            problems.append(f"relationship_reconciliations[{index}].fact_paths: must equal the complete ordered request fact path list")
        status = row.get("status")
        if expected_compat == "compatible" and status != "reconciled":
            problems.append(f"relationship_reconciliations[{index}].status: must be 'reconciled' for request compatibility 'compatible'")
        if expected_compat == "incompatible":
            if row.get("summary_synthesis"):
                problems.append(f"relationship_reconciliations[{index}].summary_synthesis: must be empty for incompatible relationship")
            if row.get("thesis_synthesis"):
                problems.append(f"relationship_reconciliations[{index}].thesis_synthesis: must be empty for incompatible relationship")
            if row.get("summary_fact_paths"):
                problems.append(f"relationship_reconciliations[{index}].summary_fact_paths: must be empty for incompatible relationship")
        if status == "reconciled":
            s_synth = row.get("summary_synthesis") or ""
            t_synth = row.get("thesis_synthesis") or ""
            s_paths = row.get("summary_fact_paths") or []
            if not (1 <= len(s_paths) <= 2) or len(set(s_paths)) != len(s_paths) or any(p not in expected_paths for p in s_paths):
                problems.append(f"relationship_reconciliations[{index}].summary_fact_paths: must be unique 1-2 subset of fact_paths")
            if s_synth and _normalize_grounding_text(s_synth) not in summary_text:
                problems.append(f"relationship_reconciliations[{index}].summary_synthesis: must appear in summary")
            if t_synth and _normalize_grounding_text(t_synth) not in thesis_text:
                problems.append(f"relationship_reconciliations[{index}].thesis_synthesis: must appear in thesis")
    return problems
