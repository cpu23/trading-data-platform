"""Durable job repository with caller-owned transactions.

Jobs are the durable hand-off between acceptance and worker execution.
``accept_and_enqueue_operation`` inserts a ``cycle_runs`` acceptance row and
its ``jobs`` entry in one caller-owned transaction, so acceptance and enqueue
either both persist or neither does. Lease claim, heartbeat renewal,
retry/backoff, poison terminal state, and expired-lease reclaim all use this
single queue.

Duplicate logical runs are prevented by the partial unique index
``idx_jobs_active_identity`` plus an advisory transaction lock keyed
...
_OPERATION_FIELDS = """id, run_kind, requested_component, correlation_id, state, priority,
...
def _operation_identity_select(
    ...
    return result_first(
        session.execute(
            text(
                f"SELECT {_OPERATION_FIELDS} FROM jobs "
...
def enqueue_operation(
    ...
    *,
    ...
    if not str(dedupe_key).strip() or not str(input_fingerprint).strip():
    ...
    existing = _operation_identity_select(session, identity)
    if existing is not None:
        return OperationJobEnqueueResult(OperationJob.from_row(existing), False, True)
    params = {
        "id": str(uuid4()),
        **identity,
    ...
    row = result_first(
        session.execute(
            text(
                "INSERT INTO jobs (id, run_kind, requested_component, "
    ...
    with orchestrator.get_session(config) as session:
        ...
        if getattr(inserted, "rowcount", 0) == 0:
            ...
            if existing is not None:
                ...
                # acceptance.
                if str(existing.get("triggered_by") or "") == "api_manual_override":
                    session.execute(
                        text(
                            ...
                            + _jsonb_expr("empty_summary", session)
                            + ") || "
                            + _jsonb_expr("summary", session)
                            + " WHERE correlation_id = :cid"
                        ),
        ...
        if not enqueued.inserted:
            ...
            summary = {
                ...
                "prior_correlation_id": str(getattr(prior, "correlation_id", "") or ""),
            }
            session.execute(
                text(
...
def claim_operation_jobs(
    ...
    where = (
        "run_kind IS NOT NULL AND ((state IN ('queued','failed_retryable') AND not_before <= :now) "
    ...
    rows = result_rows(
        session.execute(
            text(
                f"SELECT id, correlation_id FROM jobs WHERE {where} "
...
def succeed_operation_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'succeeded', completed_at = :now, "
...
def retry_operation_job(
    ...
    updated = result_first(
        session.execute(
            text(
                "UPDATE jobs SET state = 'failed_retryable', "
...
def terminal_fail_operation_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'failed_terminal', completed_at = :now, "
...
def renew_operation_job_lease(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET lease_expires_at = :lease_until "
    ...
    candidates = result_rows(
        session.execute(
            text(
                ...
                "FROM jobs "
                "WHERE run_kind IS NOT NULL AND state IN ('leased','running') AND lease_expires_at <= :now "
    ...
    with orchestrator.get_session(config) as session:
        rows = session.execute(
            text(
                ...
                "FROM jobs GROUP BY state ORDER BY state"
            )
        ).mappings()
    ...
    if active_rows:
        with orchestrator.get_session(config) as session:
            row = (
                session.execute(
                    text(
                        "SELECT MIN(created_at) AS oldest FROM jobs "
...
# Analysis job API; shares the same durable jobs table and lifecycle states.
@dataclass(frozen=True)
class AnalysisJob:
    id: Any
    job_type: str
...
_ANALYSIS_FIELDS = """id, job_type, state, priority, source_event_id, dedupe_key,
...
def _analysis_identity_select(
    session: Any, identity: Mapping[str, Any]
) -> Mapping[str, Any] | None:
    return result_first(
        session.execute(
            text(
                f"SELECT {_ANALYSIS_FIELDS} FROM jobs WHERE job_type = :job_type AND dedupe_key = :dedupe_key AND input_fingerprint = :input_fingerprint ORDER BY created_at DESC, id DESC LIMIT 1"
            ),
            dict(identity),
        )
    )
...
def enqueue_job(
    ...
    *,
    ...
    existing = _analysis_identity_select(session, identity)
    if existing is not None:
    ...
    row = result_first(
        session.execute(
            text(
                "INSERT INTO jobs (job_type, state, priority, source_event_id, dedupe_key, input_fingerprint, not_before, max_attempts, payload, correlation_id) VALUES (:job_type, 'queued', :priority, :source_event_id, :dedupe_key, :input_fingerprint, :not_before, :max_attempts, CAST(:payload AS JSONB), :correlation_id) ON CONFLICT DO NOTHING RETURNING "
                + _ANALYSIS_FIELDS
            ),
            params,
        )
...
def claim_jobs(
    ...
    lease_until = now + timedelta(seconds=max(1.0, float(lease_seconds)))
    params: dict[str, Any] = {"now": now, "limit": max(1, int(limit))}
    where = "job_type IS NOT NULL AND ((state IN ('queued','failed_retryable') AND not_before <= :now) OR (state = 'leased' AND lease_expires_at <= :now)) AND attempt_count < max_attempts"
    if job_types:
        names = []
    ...
    rows = result_rows(
        session.execute(
            text(
                f"SELECT id FROM jobs WHERE {where} ORDER BY priority DESC, not_before ASC, created_at ASC, id ASC LIMIT :limit"
                + lock
            ),
            params,
        )
    ...
    for row in rows:
        updated = result_first(
            session.execute(
                text(
                    "UPDATE jobs SET state = 'leased', claimed_by = :worker_id, lease_expires_at = :lease_until, attempt_count = attempt_count + 1 WHERE id = :id AND attempt_count < max_attempts AND ((state IN ('queued','failed_retryable') AND not_before <= :now) OR (state = 'leased' AND lease_expires_at <= :now)) RETURNING "
                    + _ANALYSIS_FIELDS
                ),
                {
                    "id": row["id"],
...
def start_job(session: Any, job_id: Any, worker_id: str) -> bool:
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'running', started_at = COALESCE(started_at, :now) WHERE id = :id AND state = 'leased' AND claimed_by = :worker_id AND lease_expires_at > :now"
        ),
...
def succeed_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'succeeded', completed_at = :now, claimed_by = NULL, lease_expires_at = NULL, result_ref = CAST(:result_ref AS JSONB) WHERE id = :id AND state = 'running' AND claimed_by = :worker_id"
        ),
        {
            "id": job_id,
            "worker_id": worker_id,
...
def retry_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'failed_retryable', not_before = :not_before, claimed_by = NULL, lease_expires_at = NULL, last_error = :last_error WHERE id = :id AND state = 'running' AND claimed_by = :worker_id AND attempt_count < max_attempts"
        ),
        {
            "id": job_id,
            "worker_id": worker_id,
...
def terminal_fail_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = 'failed_terminal', completed_at = :now, claimed_by = NULL, lease_expires_at = NULL, last_error = :last_error WHERE id = :id AND state = 'running' AND claimed_by = :worker_id"
        ),
        {
            "id": job_id,
            "worker_id": worker_id,
...
def suppress_job(
    ...
    result = session.execute(
        text(
            "UPDATE jobs SET state = :suppression_state, completed_at = :now, claimed_by = NULL, lease_expires_at = NULL WHERE id = :id AND state IN ('queued','leased','running','failed_retryable')"
            + guard
        ),
        params,
    )
...
def reconcile_jobs(session: Any, limit: int = 100) -> int:
    result = session.execute(
        text(
            "UPDATE jobs SET state = CASE WHEN attempt_count >= max_attempts THEN 'failed_terminal' ELSE 'failed_retryable' END, not_before = :now, claimed_by = NULL, lease_expires_at = NULL, completed_at = CASE WHEN attempt_count >= max_attempts THEN :now ELSE completed_at END WHERE id IN (SELECT id FROM jobs WHERE job_type IS NOT NULL AND state IN ('leased','running') AND lease_expires_at <= :now ORDER BY lease_expires_at ASC LIMIT :limit)"
        ),
...
def run_job_worker_forever(
    config: dict[str, Any], *, stop_event: Any | None = None
) -> None:
    """Run both job handlers behind one worker lifecycle."""
    import threading
    import time
    ...
    try:
        ...
        job_worker.stop(timeout=15.0)
        operation_worker.stop(timeout=15.0)
