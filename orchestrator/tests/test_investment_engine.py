...
def facts(**values):
    metrics = {
        name: {"value": value, "unit": "currency", "period": "FY2025", "evidence": [name]}
...
class InvestmentEngineTests(unittest.TestCase):
    def test_fcf_and_margin_use_positive_capex_outflow(self):
        result = build_deterministic_analysis(facts(revenue=200, operating_cash_flow=80, capex=20))
    ...
    def test_dcf_projects_five_years_and_caps_inferred_growth(self):
        current = facts(revenue=200, operating_cash_flow=200, capex=0, net_debt=10, shares_outstanding=10)
        previous = facts(revenue=100, operating_cash_flow=100, capex=0)
        result = build_deterministic_analysis(current, previous, {"discount_rate": 0.10, "terminal_growth": 0.03})
    ...
    def test_pe_uses_eps_then_market_cap_net_income_fallback(self):
        ...
        fallback = build_deterministic_analysis(facts(market_price=100, shares_outstanding=10, diluted_eps=0, net_income=50))
    ...
    def test_zero_and_malformed_values_remain_missing(self):
        result = build_deterministic_analysis(
            facts(revenue=0, operating_cash_flow="nan", capex=10, diluted_eps=0, shares_outstanding=0)
        )
    ...
    def test_threshold_boundaries_are_explicit(self):
        ...
        at_ten = build_deterministic_analysis(facts(revenue=110, operating_cash_flow=11, capex=1), prior)
        just_under = build_deterministic_analysis(facts(revenue=109.99, operating_cash_flow=11, capex=1), prior)
    ...
    def test_dram_leading_signals_compound(self):
        result = build_deterministic_analysis(
            {
                **facts(revenue=120, operating_cash_flow=60, capex=10, backlog=150, inventory=105, gross_margin=55),
                "qualitative": {
                    "ai_demand": {"present": True, "strength": "strong", "evidence": ["ai"]},
                    "data_centre_demand": {"present": True, "strength": "strong", "evidence": ["dc"]},
                    "pricing_power": {"present": True, "strength": "strong", "evidence": ["price"]},
                },
            },
            facts(revenue=100, operating_cash_flow=40, capex=10, backlog=100, inventory=100, gross_margin=50),
        )
    ...
    def test_mature_requires_repeated_evidence_valuation_and_news(self):
        current = {
            **facts(revenue=120, operating_cash_flow=60, capex=10, backlog=150, gross_margin=55, market_price=100, diluted_eps=2, shares_outstanding=10, net_debt=0),
            "qualitative": {"ai_demand": {"present": True, "strength": "strong"}, "pricing_power": {"present": True, "strength": "strong"}},
        }
        previous = facts(revenue=100, operating_cash_flow=40, capex=10, backlog=100, gross_margin=50)
        result = build_deterministic_analysis(current, previous, previous_state="accelerating", prior_analysis_count=2, news_items=[1, 2, 3])
        self.assertEqual(result["state"]["stage"], "mature")
        not_repeated = build_deterministic_analysis(current, previous, previous_state="accelerating", prior_analysis_count=1, news_items=[1, 2, 3])
