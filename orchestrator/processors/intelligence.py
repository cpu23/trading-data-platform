...
class MarketIntelligenceProcessor:
    ...
    def _role_prompt(self, role, instruction, context):
        ...
        return (
            ...
            "An evidence_id must entail the complete claim, not merely share its topic. "
            "Do not add relative-growth, policy-divergence, market-level, or causal "
            "premises absent from the cited evidence value. Economic-event evidence "
            "describes a future schedule and consensus, not an observed outcome. It "
            "may support only an explicitly conditional future-risk claim, never a "
            "claim that the release occurred or sole support for current directional "
            "bias. Treat dated evidence conservatively; do not call it current when "
            "freshness cannot be established. "
            "Asset context is an eligibility map, not evidence that any channel is "
            "active. Relative-growth and policy- or yield-differential claims require "
            "current evidence for both sides. Broad US growth does not establish "
            "Eurozone, German, UK, Japanese or Australian conditions. A breakeven-"
            "inflation series is not a nominal or real yield; a flat dollar is not a "
            "stronger or weaker dollar. Do not add a period label absent from evidence. "
            "Do not group gold with platinum or treat platinum as a safe haven. A "
            "risk-on regime alone does not establish an Australia-Japan or US-Japan "
            "yield or policy differential. "
            "The broad US macro regime may support SP500 through US growth, credit, "
            "and systemic-stress channels. It does not by itself establish a direction "
            "for a currency pair, DXY, a foreign index, gold or platinum; those assets "
            "need current evidence explicitly establishing the relevant relative, "
            "regional, dollar, real-rate or demand direction. Do not compare a current "
            "observed rate with scheduled-event consensus as the same series or a "
            ...
            "Use the supplied asset context only to determine whether economic "
            "evidence can reach an asset through an allowed channel. CFTC evidence "
            "may only be applied to assets listed for that market_id. Do not infer a "
            "contract identity from its numeric market_id. Include an asset claim only "
            "when supplied evidence both activates a listed channel and establishes "
            "its directional sign. Otherwise use an empty claim array. A positioning "
            "claim may describe exactly one participant category and one positioning "
            "evidence_id; never combine dealers, asset managers or leveraged funds "
            "into one directional claim. "
            ...
            "Bias, confidence, claims and contradictions must agree. Use mixed only "
            "when cited current evidence establishes opposing directional forces. "
            "Use neutral and low confidence when no retained claim establishes a "
            "direction. High confidence requires multiple independent current "
            "evidence records; one macro-regime opinion alone permits at most "
            "moderate confidence. A future event does not count as current evidence. "
    ...
    def _editor_prompt(self, context, roles):
        ...
        return (
            ...
            "Each cited source claim must support the complete narrative sentence, "
            "not merely its topic. Never combine claim fragments into a new fact or "
            "causal premise. Economic-event claims are conditional future risks: use "
            "them only in invalidation_conditions or disagreements, never as current "
            "drivers or evidence that an outcome occurred. Contradictions must "
            "describe currently supplied counterevidence. Invalidation conditions "
            "must describe what would negate a cited driver and must not imply that "
            "the condition has already occurred. "
            "The asset-channel map is not evidence. Discard any relative-growth, "
            "policy-differential, yield-differential, real-yield, regional-growth or "
            "demand claim unless the cited source claim contains the required facts "
            "for every side of that comparison. Treat breakeven inflation as neither "
            "a nominal nor real yield, and treat a flat dollar as neither stronger nor "
            "weaker. "
            "Discard currency-pair, DXY, foreign-index, gold and platinum direction "
            "derived only from a broad US macro-regime claim. The broad regime may "
            "support SP500 only through cited US growth, credit or systemic-stress "
            "facts. "
            ...
            "Derive bias and confidence from retained current drivers rather than "
            "copying a role label. Use mixed only when cited current drivers oppose "
            "one another; use neutral and low confidence when no driver establishes "
            "a direction. High confidence requires multiple independent current "
            "evidence records; one macro-regime opinion alone permits at most "
            "moderate confidence. Summary, drivers, contradictions and bias must be "
            "logically consistent. Every non-neutral assessment must include at least "
            "one explicit invalidation condition tied to a retained driver; neutral "
            "assessments may use an empty array. Before returning JSON, silently "
            "verify that each driver is entailed by its evidence, each causal sign "
            "follows the supplied channel rules, each bias follows its retained "
            "drivers, and every directional assessment has an invalidation. "
            ...
            for field in (
                ...
                for item in items:
                    ...
                    if item.get("source_claim_ids") and item.get("evidence_ids"):
                        kept.append(item)
                assessment[field] = kept
            if scope != "global" and not assessment.get("drivers"):
                summary = assessment.get("summary")
                if isinstance(summary, dict):
                    summary["text"] = (
                        "Insufficient direct evidence for a distinct asset assessment."
                    )
                    summary["source_claim_ids"] = []
