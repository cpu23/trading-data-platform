...
class ProcessorConfig(FrozenModel):
    ...
    # Runtime-consumed knobs: briefing atom cap.
...
KNOWN_PROCESSORS = frozenset({"macro_regime", "event_impact", "briefing"})
...
class AppConfig(FrozenModel):
    ...
    def _validate_collectors(
        cls, value: dict[str, CollectorConfig]
    ) -> dict[str, CollectorConfig]:
        unknown_collectors = set(value) - KNOWN_COLLECTORS
    ...
    def _validate_processors(
        cls, value: dict[str, ProcessorConfig]
    ) -> dict[str, ProcessorConfig]:
        unknown_processors = set(value) - KNOWN_PROCESSORS
