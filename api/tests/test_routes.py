...
class TestFinancialTimesRoutes(unittest.TestCase):
    """Integration tests for /api/collect/financial-times trigger endpoint."""

    @patch("routes.json.triggers.httpx.AsyncClient.post")
    def test_trigger_financial_times_returns_202(self, mock_post):
        """POST /api/collect/financial-times with a successful orchestrator returns 202."""
        mock_response = MagicMock()
        mock_response.status_code = 202
        mock_response.json.return_value = {
            "job_id": "test-ft-job-id",
            "accepted_at": "2026-07-11T12:00:00+00:00",
        }
        mock_post.return_value = mock_response

        resp = client.post("/api/financial-times", json={}, headers=AUTH)
    ...
    @patch("routes.json.triggers.httpx.AsyncClient.post")
    def test_trigger_financial_times_forwards_body(self, mock_post):
        """POST with body parameters should forward them to orchestrator."""
        mock_response = MagicMock()
        mock_response.status_code = 202
        mock_response.json.return_value = {
            "job_id": "ft-body-job",
        ...
        resp = client.post(
            "/api/financial-times",
            json={"sections": ["lex"], "max_articles": 3},
            headers=AUTH,
        )
        self.assertEqual(resp.status_code, 202)

    @patch("routes.json.triggers.httpx.AsyncClient.post")
    def test_trigger_financial_times_orchestrator_unavailable(self, mock_post):
        """When orchestrator is down, the endpoint returns 503."""
    ...
    @patch("routes.json.triggers.httpx.AsyncClient.post")
    def test_trigger_financial_times_concurrent_returns_409(self, mock_post):
        """When orchestrator returns 409, the API forwards it."""
        mock_response = MagicMock()
        mock_response.status_code = 409
        mock_response.json.return_value = {
            "detail": "FT collection already running: abc-123",
        }
        mock_post.return_value = mock_response
