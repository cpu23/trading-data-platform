...
class TestSystemRoutes(unittest.TestCase):
    ...
    def test_degraded_quality_without_source_id_degrades_api_health(
        ...
        quality_component = next(
            c for c in data["components"] if c["name"] == "quality_checks"
        )
...
class TestBoundedDashboardInputs(unittest.TestCase):
    ...
    def test_calendar_defaults_custom_values_and_upper_clamps_reach_one_bounded_query(
        self, query
    ):
        for url, expected in (
            ...
            with self.subTest(url=url):
                ...
                self.assertEqual(response.status_code, 200)
                self.assertEqual(
                    (response.json()["hours"], response.json()["limit"]), expected
                )
        ...
        for name in ("hours", "limit"):
            for value in ("wat", "1.5", "0", "-1", "+2"):
                with self.subTest(name=name, value=value):
                    response = client.get(
                        f"/api/calendar/events?{name}={value}", headers=AUTH
                    )
                    self.assertEqual(response.status_code, 422)
        query.assert_not_called()

    @patch("routes.json.events.query_many")
    def test_calendar_renders_selected_timezone_only_after_cookie_allowlist(
        self, query
    ):
        query.return_value = [
            {
                ...
                "event_name": "Open",
                "country": "JP",
                "scheduled_at": datetime(2026, 7, 16, 0, 0, tzinfo=UTC),
            }
        ]
    ...
    def test_api_logs_lines_default_custom_and_clamp(self, query):
        for url, expected in (
            ("/api/logs", 200),
            ("/api/logs?lines=12", 12),
            ("/api/logs?lines=5000", 1000),
        ):
            with self.subTest(url=url):
                query.reset_mock()
                response = client.get(url, headers=AUTH)
...
class TestTimezoneSettings(unittest.TestCase):
    ...
    def test_post_accepts_allowlisted_timezone_and_sets_strict_cookie(self):
        response = client.post(
            "/api/settings/timezone", headers=AUTH, json={"timezone": "Asia/Tokyo"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["current"], "Asia/Tokyo")
        cookie = response.headers["set-cookie"]
        self.assertEqual(
            response.cookies.get("display_timezone").strip('"'), "Asia/Tokyo"
        )
        self.assertEqual(
            client.get("/api/settings/timezone", headers=AUTH).json()["current"],
            "Asia/Tokyo",
        )
...
class TestOperationsOverview(unittest.TestCase):
    ...
    def test_operations_dispatches_local_snapshot_to_threadpool(
        self, threadpool, _health
    ):
    ...
    def test_populated_overview_is_bounded_and_renders_required_sections(
        self, feed, query, health
    ):
        health.return_value = {
            "components": [
                {"name": "fred", "last_status": "success", "next_due_at": "later"}
            ],
            "readiness": "ready",
        }
        query.side_effect = [
    ...
    @patch(
        "routes.views.operations.query_many",
        side_effect=[RuntimeError("RAW_DB_SECRET"), []],
    )
    @patch(
        "routes.views.operations._feed_snapshot",
        side_effect=RuntimeError("RAW_FEED_SECRET"),
    )
    def test_partial_failure_and_empty_sections_fail_soft_without_raw_errors(
        self, _feed, _query, _health
    ):
        response = client.get("/operations", headers=AUTH)
        self.assertEqual(response.status_code, 200)
...
class TestMacroRoutes(unittest.TestCase):
    ...
    @patch(
        "routes.json.macro.query_many",
        return_value=[{"observed_at": "2026-07-01T00:00:00+00:00", "value": 16.0}],
    )
...
class TestHealthContract(unittest.TestCase):
    ...
    def test_health_returns_contract_shape(self, mock_get, _mock_qm):
        ...
        self.assertIn("liveness", data, "Response must include liveness key")
        self.assertIn("readiness", data, "Response must include readiness key")
        self.assertIn("data_health", data, "Response must include data_health key")
        self.assertIn("components", data, "Response must include components key")
    ...
    def test_stale_data_yields_liveness_ok_data_health_degraded(
        ...
        self.assertEqual(
            data["liveness"], "ok", "Liveness stays 'ok' even when no data available"
        )
        self.assertEqual(data["data_health"], "degraded")
...
class TestCalendarDisplayTimezone(unittest.TestCase):
    def test_serialized_time_and_day_boundary_use_selected_timezone(self):
        ...
        row = {
            ...
            "country": "JP",
            "scheduled_at": datetime(2026, 1, 1, 15, 30, tzinfo=UTC),
            "impact_level": "high",
            "consensus": None,
            "previous": None,
            "actual": None,
