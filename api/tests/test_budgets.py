...
class ApiBudgetTests(unittest.TestCase):
    def test_spend_query_uses_bounded_utc_day_and_null_safe_tokens(self):
        ...
        with patch("budgets.query_one", return_value={"total_cost": None, "total_tokens": None}) as query:
    ...
    def test_api_reports_lookup_failure_as_unavailable_not_zero(self):
        ...
        for spend_result in (
            ...
            with self.subTest(spend_result=spend_result), patch(
                "budgets.get_today_spend",
                side_effect=spend_result if isinstance(spend_result, Exception) else None,
                return_value=None if isinstance(spend_result, Exception) else spend_result,
            ):
    ...
    def test_invalid_cap_is_reported_unavailable_and_never_unlimited(self):
        for cap in ("bad", None, math.nan, math.inf, -math.inf, True):
            with self.subTest(cap=cap), patch("budgets.get_today_spend", return_value=(0, 0)):
                status = get_budget_status({"budgets": {"daily_llm_usd": cap, "warn_at_pct": 80}})
                self.assertFalse(status["available"])
