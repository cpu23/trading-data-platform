...
from api_db import get_session, query_one
...
def extract_manual_override(
    ...
    if not isinstance(body["budget_override"], bool):
        raise HTTPException(status_code=422, detail="budget_override must be a boolean")
    if body["budget_override"] is not True:
        return None

    reason = body.get("override_reason")
    if not isinstance(reason, str):
        raise HTTPException(status_code=422, detail="override_reason must be a string")
    reason = reason.strip()
